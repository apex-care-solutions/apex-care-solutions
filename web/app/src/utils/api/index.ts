...
export const createResponse = <T = any>({status, data, error, redirect}:{status: keyof typeof HTTP_STATUS_CODES, data?: T, error?: string, redirect?: string}) => ({...HTTP_STATUS_CODES[status], data, error, redirect})
